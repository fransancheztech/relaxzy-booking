...
export async function GET(request: Request) {
  ...
  try {
    ...
    const [
      ...
      // 2. Voucher revenue
      prisma.$queryRaw<{ voucher: number }[]>`
      ...
      // 3. Revenue over time (payment_events)
      prisma.$queryRaw<{ period: Date; cash: number; credit_card: number; refunds: number }[]>`
        SELECT
      ...
      // 4. Voucher revenue over time
      prisma.$queryRaw<{ period: Date; voucher: number }[]>`
        SELECT
      ...
      // 6. By service
      prisma.$queryRaw<{ service_name: string; count: number; revenue: number }[]>`
      ...
      // 7. By day of week
      prisma.$queryRaw<{ day_of_week: number; count: number }[]>`
      ...
      // 9. By duration
      prisma.$queryRaw<{ duration_minutes: number; count: number }[]>`
        SELECT
      ...
      // 10. Financial KPIs
      prisma.$queryRaw<{ avg_ticket: number; p25: number; p75: number }[]>`
        SELECT
      ...
      // 11. All prices for ticket distribution
      prisma.$queryRaw<{ price: number }[]>`
        SELECT price::float FROM bookings
        WHERE deleted_at IS NULL AND status = 'completed' AND price IS NOT NULL
          AND start_time >= ${from} AND start_time < ${to}
      `,

      // 12. Total unique clients in period
      prisma.$queryRaw<{ total_unique: number }[]>`
        SELECT COUNT(DISTINCT client_id)::int AS total_unique FROM bookings
        WHERE deleted_at IS NULL AND start_time >= ${from} AND start_time < ${to}
          AND client_id IS NOT NULL
      `,

      // 13. Total clients all-time
      prisma.$queryRaw<{ total_all_time: number }[]>`
        SELECT COUNT(*)::int AS total_all_time FROM clients WHERE deleted_at IS NULL
      `,

      // 14. New clients in period (first-ever booking in period)
      prisma.$queryRaw<{ new_in_period: number }[]>`
      ...
      // 15. Avg bookings per client
      prisma.$queryRaw<{ avg_bookings_per_client: number }[]>`
      ...
      // 16. Repeat frequency (avg days between sessions for multi-visit clients)
      prisma.$queryRaw<{ repeat_frequency_days: number }[]>`
      ...
      // 17. New clients over time
      prisma.$queryRaw<{ period: Date; count: number }[]>`
